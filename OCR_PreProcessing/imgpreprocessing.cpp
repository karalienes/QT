...
ImgPreProcessing::ImgPreProcessing()
{
    colorImg = imread("/home/karali/image/TestOCR/of2.png");
    getContours(colorImg);
...
void ImgPreProcessing::Binarization()
{
    Mat gausssianImg;
...
void ImgPreProcessing::skewCorrention()
{
    bitwise_not(colorImg,colorImg);
    imshow("skewCorrention",colorImg);
}

void ImgPreProcessing::deskew(Mat &imgIn, double angle)
{

}

double ImgPreProcessing::computeSkewByAreaBox(Mat &imgIn)
{

}

double ImgPreProcessing::computeSkewByHoughLines(Mat &imgIn)
{

}

void ImgPreProcessing::getContours(Mat &imgIn)
{
...
//     original_image = imread("/home/karali/image/TestOCR/of.png");//loading image into matrix//
...
//    waitKey(0);
    ...
    adaptiveThreshold(img, img, 255, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY_INV, 55, 10);
    bitwise_not(img,img);
    bitwise_not(img, img);
...
void ImgPreProcessing::rotate_bound(Mat &image, double angle)
{

}


bool ImgPreProcessing::isRepetitive(const string& s){
    int count = 0;
...
void ImgPreProcessing::er_draw(vector<Mat> &channels, vector<vector<ERStat> > &regions, vector<Vec2i> group, Mat& segmentation){
...
std::vector<Mat> ImgPreProcessing::extractTableFromImage(Mat src){
...
Mat ImgPreProcessing::labelPreprocessing(Mat image){
    Mat grey;
...
void ImgPreProcessing::endToEndSceneTextDetectionAndRecognition(Mat image){
